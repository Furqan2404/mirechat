...
class ConnectPage(GridLayout):
	...
	def connect(self, _):
		...
		ip = self.ip.text
		username = self.username.text
		if not Sockets_client.connect(ip, port, username, show_error):
			return 

		ChatApp.CreateChatPage()
...
class InfoPage(GridLayout):
	def __init__(self, **kwargs):
		...
		self.message.bind(width=self.update_text_with)
		self.add_widget(self.message)

	def update_info(self, message):
		self.message.text = message

	def update_text_with(self,*_):
...
class ChatPage(GridLayout):
	def __init__(self, **kwargs):
		...
		Sockets_client.start_listening(self.incoming_message, show_error)
